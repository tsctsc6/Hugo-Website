...
适用场景：大列表、高并发、需要极致性能的系统，一般系统也能使用，是比较好的解决方案。

LexoRank 这种方法，就像是手写小数一样（区别于计算机内部的固定大小的浮点数），可以有无限精度（只要你的数据库塞得下）。另外， LexoRank 虽然要和浮点数法一样，时不时要重平衡，但是：

1. LexoRank 的重平衡的频率比浮点数法要低；
1. 要是你愿意，甚至可以不重平衡，只是影响索引的效率；而浮点数法是必须要重平衡的。

LexoRank 算法通常遵循以下规则： **永远不使用最小字符作为结尾** 。因为 LexoRank 算法就像手写小数一样，在小数的角度来看， "b", "ba", "baa", ... 是等价的（这里 "a" 是最小字符；就像小数 0.1 和 小数 0.10 是等价的一样），于是不能在 "b", "ba" 之间找到一个值。如果算法算出最小字符作为结尾的值，自动追加一个中间字符即可。
